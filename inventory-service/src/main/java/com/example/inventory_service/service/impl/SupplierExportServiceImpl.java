...
public class SupplierExportServiceImpl implements SupplierExportService {
    ...
    public SupplierExportDto create(SupplierExportRequest req) {
        ...
        for (ExportDetailRequest item : req.getItems()) {
            ...
            d.setDiscountPercent(item.getDiscountPercent()); // Lưu % chiết khấu
    ...
    public SupplierExportDto update(Long id, SupplierExportRequest req) {
        ...
        for (ExportDetailRequest item : req.getItems()) {
            ...
            d.setQuantity(item.getQuantity());
            d.setUnitPrice(item.getUnitPrice());
            d.setDiscountPercent(item.getDiscountPercent()); // ✅ THÊM DÒNG NÀY
    ...
    private SupplierExportDto toDto(ShopExport e, BigDecimal total) {
        ...
        // Lấy thông tin store (NGUỒN XUẤT = kho xuất hàng)
        ...
        // Lấy thông tin supplier (đích đến = nhà cung cấp/khách hàng)
        if (e.getSupplierId() != null) {
            try {
                var supplierInfo = productClient.getSupplier(e.getSupplierId());
                if (supplierInfo != null) {
                    dto.setSupplierName(supplierInfo.getName());
                }
            } catch (Exception ex) {
                System.err.println("❌ Failed to get supplier info for supplierId: " + e.getSupplierId());
